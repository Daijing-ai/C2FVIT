...
from Code.C2FViT_model import C2F_ViT_stage, AffineCOMTransform, Center_of_mass_initial_pairwise, multi_resolution_NCC, multi_resolution_NCC_with_weighted_sum
...
def train():
    ...
    #when use weightedNCC (and scale=4) as loss function 
    # loss_similarity = multi_resolution_NCC_with_weighted_sum(win=7, scale=4, weights=[0.4, 0.3, 0.2, 0.1])

    imgs = sorted(glob.glob(datapath + "\\image\\p_*.nii.gz"))
    labels = sorted(glob.glob(datapath + "\\label\\p_*.nii.gz"))

    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    
    #when use SGD as optimizer
    # optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9)
    model_dir = "D:\\project\\regis\\C2FViT-main\\Model" + model_name[0:-1]
    ...
    while step <= iteration:
        for X, Y in training_generator:
            ...
            if (step % n_checkpoint == 0):
                ...
                test_imgs = sorted(glob.glob("D:\\project\\regis\\OASIS\\OASIS_L2R_2021_task03\\Test_nii" + "\\image\\img*.nii.gz"))[10:15]
                test_labels = sorted(glob.glob("D:\\project\\regis\\OASIS\\OASIS_L2R_2021_task03\\Test_nii" + "\\label\\seg*.nii.gz"))[10:15]
                
                valid_generator = Data.DataLoader(
                    Dataset_epoch(test_imgs, test_labels, norm=True, use_label=True),
                    batch_size=1,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--datapath", type=str,
                        dest="datapath",
                        default="D:\\project\\regis\\OASIS\\OASIS_L2R_2021_task03\\train_nii",

...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=128, patch_size=7, stride=4, in_chans=3, embed_dim=768, flatten=True):
        ...
        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=patch_size, stride=stride,
                              padding=(patch_size[0] // 2, patch_size[1] // 2, patch_size[2] // 2))        # padding = patch_size // 2
...
class C2F_ViT_stage(nn.Module):
    ...
    def forward(self, x, y):
        ...
        for i in range(self.num_stages):
            ...
            xy_patch_embed, H, W, D = patch_embed_xy(xy)                  #[1, 1680, 256]
            ...
            affine = head(xy_fea.mean(dim=1))         #[1, 12]
...
class Center_of_mass_initial_pairwise(nn.Module):                  
    ...
    def forward(self, x, y):             # x.shape & y.shape=[1, 1, 160, 192, 224]
...
class multi_resolution_NCC(torch.nn.Module):        
    ...
    local (over window) normalized cross correlation
    """
    def  __init__(self, win=None, eps=1e-5, scale=4, kernel=3):
...
class multi_resolution_NCC_with_weighted_sum(torch.nn.Module):

    def __init__(self, win=None, eps=1e-5, scale=4, kernel=3, weights=None):
        super(multi_resolution_NCC_with_weighted_sum, self).__init__()
        self.num_scale = scale
        self.kernel = kernel
        self.similarity_metric = []     
        self.weights = weights if weights else [1.0 / scale] * scale
    ...
    def forward(self, I, J):
        ...
        for i in range(self.num_scale):
            current_NCC = self.similarity_metric[i](I, J)
            total_NCC += self.weights[i] * current_NCC/(2**i)
